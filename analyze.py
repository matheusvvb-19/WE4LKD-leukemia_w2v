...
def create_pdf_header(common_number, filenames, n = 15):
    pdf = FPDF()   
    pdf.add_page()
    ...
    if common_number > 0:
        pdf.cell(200, 10, txt = 'Palavras comuns removidas: {}'.format(common_number), ln = 2, align = 'C')
    ...
    pdf.multi_cell(0, 5, txt = '1. em todos os modelos, imprimir as {} palavras mais próximas a "cytarabine" e "daunorubicin", caso elas existam no vocabulário do modelo;'.format(n), align = 'L')
    pdf.multi_cell(0, 5, txt = '2. em cada modelo, buscar sinônimos de ativos ainda não descobertos na época, de acordo com a linha do tempo base. Caso os termos estejam no vocabulário, imprimir sua distância até "cytarabine" e "daunorubicin", e imprimir suas {} palavras mais próximas.'.format(n), align = 'L')
...
def investigate_models(pdf, model, model_year, i, n):
    ...
    if model_year < 1968:
        pdf.multi_cell(0, 5, 'buscar pela presenca de ativos descobertos a partir de 1968: Cytarabine, Daunorubicin, Azacitidine, Gemtuzumab Ozogamicine, Midostaurin, CPX-351, Ivosidenib, Venetoclax, Enasidenib, Gilteritinib e Glasdegib.', align='L')
        ...
        pdf.multi_cell(0, 5, 'buscar pela presenca de ativos descobertos a partir de 1978: Azacitidine, Gemtuzumab Ozogamicine, Midostaurin, CPX-351, Ivosidenib, Venetoclax, Enasidenib, Gilteritinib e Glasdegib.', align='L')
        ...
        pdf.multi_cell(0, 5, 'buscar pela presenca de ativos descobertos a partir de 2000: Gemtuzumab Ozogamicine, Midostaurin, CPX-351, Ivosidenib, Venetoclax, Enasidenib, Gilteritinib e Glasdegib.', align='L')
        search_targets_1968(pdf, model, flag, n, 1)
        flag = search_targets_2000(pdf, model, flag, n)
        flag = search_targets_2002(pdf, model, flag, n)
        flag = search_targets_2010(pdf, model, flag, n)
        ...
        pdf.multi_cell(0, 5, 'buscar pela presenca de ativos descobertos a partir de 2002: Midostaurin, CPX-351, Ivosidenib, Venetoclax, Enasidenib, Gilteritinib e Glasdegib.', align='L')
        search_targets_1968(pdf, model, flag, n, 1)
        flag = search_targets_2002(pdf, model, flag, n)
        flag = search_targets_2010(pdf, model, flag, n)
        flag = search_targets_2012(pdf, model, flag, n)
    ...
    if flag == 0:
        pdf.cell(0, 10, 'Sem indício de conhecimento latente neste modelo.', ln=2, align='L')
...
def search_targets_1968(pdf, model, flag, n = 15, passado=0):
    ...
    if passado == 0:
        for s in synonyms_1968:
            if s.lower() in model.wv.vocab:
                ...
                if s.lower() == 'cytarabine' or s.lower() == 'daunorubicin':
                    ...
                    pdf.multi_cell(0, 5, '\nIndício de conhecimento latente: {} está no vocabulário, é sinônim de Cytarabine ou Daunorubicin.'.format(s), align='L')
        ...
        if 'cytarabine' in model.wv.vocab:
            pdf.cell(0, 4, ' ', ln=2, align='L')
            pdf.cell(0, 5, '{} palavras próximas a cytarabine:'.format(n), ln=2, align='C')
            near = []
            near = model.wv.most_similar(positive=['cytarabine'], topn = n)
            for token, prox in near:
                table.append(tuple((token, round(float(prox), 2))))
            ...
            for row in table:
                ...
                pdf.ln(5)
        if 'daunorubicin' in model.wv.vocab:
            table = [['Token', 'Similaridade Cosseno']]
            pdf.cell(0, 4, ' ', ln=2, align='L')
            pdf.cell(0, 5, '{} palavras próximas a daunorubicin:'.format(n), ln=2, align='C')
            near = []
            near = model.wv.most_similar(positive=['daunorubicin'], topn = n)
            for token, prox in near:
                table.append(tuple((token, round(float(prox), 2))))
...
def search_targets_1978(pdf, model, flag, n = 15):
    ...
    for s in synonyms_1978:
        if s.lower() in model.wv.vocab:
            ...
            sim_cytarabine = model.wv.similarity('cytarabine', s.lower())
            sim_daunorubicin = model.wv.similarity('daunorubicin', s.lower())
            pdf.cell(0, 5, 'Similaridade cosseno entre {} e cytarabine: {}'.format(s.lower(), round(float(sim_cytarabine), 2)), ln=2, align='L')
            pdf.cell(0, 5, '{} é a {}ª palavra mais próxima a cytarabine'.format(s.lower(), model.wv.rank('cytarabine', s.lower())), ln=2, align='L')
            pdf.cell(0, 5, 'Similaridade cosseno entre {} e daunorubicin: {}'.format(s.lower(), round(float(sim_daunorubicin), 2)), ln=2, align='L')
            pdf.cell(0, 5, '{} é a {}ª palavra mais próxima a daunorubicin'.format(s.lower(), model.wv.rank('daunorubicin', s.lower())), ln=2, align='L')
            pdf.cell(0, 4, ' ', ln=2, align='L')
            pdf.cell(0, 5, '{} palavras próximas a {}:'.format(n, s), ln=2, align='C')
            near = model.wv.most_similar(positive=[s], topn = n)
            for token, prox in near:
...
def search_targets_2000(pdf, model, flag, n = 15):
    ...
    for s in synonyms_2000:
        if s.lower() in model.wv.vocab:
            flag = 1
            if s.lower() == 'gemtuzumab ozogamicina':
                pdf.multi_cell(0, 5, '\nIndício de conhecimento latente: {} está no vocabulário.'.format(s), align='L')
            else:
                pdf.multi_cell(0, 5, '\nIndício de conhecimento latente: {} está no vocabulário, é sinônim de Gemtuzumab Ozogamicin.'.format(s), align='L')
            sim_cytarabine = model.wv.similarity('cytarabine', s.lower())
            sim_daunorubicin = model.wv.similarity('daunorubicin', s.lower())
            pdf.cell(0, 5, 'Similaridade cosseno entre {} e cytarabine: {}'.format(s.lower(), round(float(sim_cytarabine), 2)), ln=2, align='L')
            pdf.cell(0, 5, '{} é a {}ª palavra mais próxima a cytarabine'.format(s.lower(), model.wv.rank('cytarabine', s.lower())), ln=2, align='L')
    ...
    for s in synonyms_2002:
        if s.lower() in model.wv.vocab:
            ...
            if s.lower() == 'midostaurin':
                ...
                pdf.multi_cell(0, 5, '\nIndício de conhecimento latente: {} está no vocabulário, é sinônim de Midostaurin.'.format(s), align='L')
            sim_cytarabine = model.wv.similarity('cytarabine', s.lower())
            sim_daunorubicin = model.wv.similarity('daunorubicin', s.lower())
            pdf.cell(0, 5, 'Similaridade cosseno entre {} e cytarabine: {}'.format(s.lower(), round(float(sim_cytarabine), 2)), ln=2, align='L')
            pdf.cell(0, 5, '{} é a {}ª palavra mais próxima a cytarabine'.format(s.lower(), model.wv.rank('cytarabine', s.lower())), ln=2, align='L')
...
n = 15
i = 1
pdf, data_hora = create_pdf_header(common_number, filenames, n)
...
pdf.output("relatorio_{}.{}.{}_{}-{}_{}.pdf".format(data_hora[0:2], data_hora[3:5], data_hora[6:11], data_hora[11:13], data_hora[14:16], common_number))